...
if (Test-Path $myagentsCache) {
    ...
    Write-Host "  ⚠️  MyAgents 缓存目录不存在 (正常，首次运行时创建)" -ForegroundColor Yellow
}
...
if ($globalSidecars) {
    ...
    Write-Host "  ⚠️  未找到 global sidecar 目录" -ForegroundColor Yellow
}
Write-Host ""
...
foreach ($port in @(31415)) {
    try {
        ...
        if ($errorMsg -match "无法连接|Unable to connect|refused") {
            ...
            Write-Host "  ⚠️  端口 $port 可连接但 /health 端点不存在" -ForegroundColor Yellow
        } else {
...
try {
    ...
    if ($firewallRules) {
        ...
        Write-Host "  ⚠️  未找到 MyAgents 防火墙规则 (可能不需要)" -ForegroundColor Yellow
    }
} catch {
...
Write-Host ""
Write-Host "请检查以上输出中的 ❌ 和 ⚠️  标记。" -ForegroundColor White
Write-Host ""
